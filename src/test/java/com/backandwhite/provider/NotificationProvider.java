...
public final class NotificationProvider {
    ...
    public static Notification failedNotification() {
        return Notification.builder().id(OTHER_NOTIFICATION_ID).recipient(OTHER_RECIPIENT).subject(SUBJECT).type(TYPE)
                .status(OTHER_STATUS).errorMessage("SMTP connection refused").retryCount(3)
